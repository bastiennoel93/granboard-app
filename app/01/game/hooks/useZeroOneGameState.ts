...
export function useZeroOneGameState(
  ...
  const [gameState, setGameState] = useState<ZeroOneGameState | null>(
    initialGameState
  );
  ...
  const handleDartHit = useCallback(
    (segment: Segment) => {
      ...
      let processedResult: ZeroOneGameState | null = null;
  ...
  const restoreGameState = useCallback(
    (state: ZeroOneGameState, turnHits: Segment[]) => {
      setGameState(state);
